...
These plugins are designed to work well together. Here are some useful combinations you can load depending on the job to be done.

| Use Case | Recommended Plugins | Example Skill Flow |
|---|---|---|
...
| Partner integration launch | `sales` + `engineering` + `legal` + `documentation` + `customer-support` | `/discovery-call` -> `/api-design` -> `/contract-review` -> `/api-docs` -> `/kb-article` |
...
| Strategic planning | `strategy` + `finance` + `data` + `product-management` | `/scenario-planning` -> `/market-entry` -> `/investment-memo` -> `/portfolio-review` -> `/roadmap-update` |
...
- `Discovery bundle`: `product-management` + `design` + `enterprise-search`
- `Revenue bundle`: `sales` + `marketing` + `finance`
- `Operations bundle`: `operations` + `security` + `documentation`
- `Platform bundle`: `devops` + `engineering` + `security`
- `Support bundle`: `customer-support` + `qa-testing` + `enterprise-search`
- `Insights bundle`: `data` + `product-management` + `documentation`
- `People bundle`: `human-resources` + `learning-development` + `productivity`
- `Leadership bundle`: `communications` + `product-management` + `finance`
- `Compliance bundle`: `legal` + `security` + `operations`
- `Growth bundle`: `marketing` + `sales` + `data`
- `Full-stack delivery`: `engineering` + `devops` + `qa-testing` + `documentation`
- `Strategy bundle`: `strategy` + `finance` + `enterprise-search`
...
Install individual plugins by path:

```bash
# Install a single plugin
claude /plugin install /path/to/skill-library/productivity

# Use --add-dir for development
claude --add-dir /path/to/skill-library/engineering

# Load a combination of plugins during development
claude \
  --add-dir /path/to/skill-library/product-management \
  --add-dir /path/to/skill-library/design \
...
1. Create a new directory at the repo root: `my-category/`
